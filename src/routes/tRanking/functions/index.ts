...
class TopModel extends Top{
    calculeRangeGlobal(){
        let rangeOfGp = [];
                ...
                let nn:number = Number(parseInt(i));
	...
	calculePositonOnGlobal(gpOfUser:number){
	...
	ranking_assign(total:number, position: number){
	...
	onReadRank(gpOfUser:number, position:number, totally: number){
